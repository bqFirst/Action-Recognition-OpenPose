...
upper_path = os.path.split(cur_path)[0]
...
def init_model():
    global ModelMap
    directory = os.path.join(upper_path, DataDName, ModelDName, ModelFileDName)
...
def model_predict(data: pd.DataFrame, model: str) -> pd.DataFrame:
    ...
    columns_map = dict([(column, 'ramadi_result{}'.format(i)) for i, column in enumerate(columns)])
    return result.rename(columns=columns_map)
...
def init_model_map():
    global ModelInfoMap
    # with open(os.path.join(DataDName, ModelMapFileName), 'r') as f:
