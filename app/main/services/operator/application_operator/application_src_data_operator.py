...
# @Author : wangweimin
# @File   : application_src_data_operator.py
# @Desc   :
...
from app.models import Application, PictureCatalog, DataSourceDataLink, DatabaseData
...
class ApplicationSrcDataService(object):

    @classmethod
    def info(cls, application_id: int):
        application: Application = ObjectAcquisition.application_by_id(application_id=application_id)
        result = []
        data_links = application.data_links.order_by(DataSourceDataLink.create_time.desc())
        for data_link in data_links:
            data_type = data_link.type
            creator = data_link.creator.name
            if 'excel' == data_type:
                for data_sheet in data_link.excel_sheets.all():
                    ...
                    data_link_info['creator'] = creator
            ...
            data_link_info['creator'] = creator
            result.append(data_link_info)
        picture_catalogs = application.picture_catalogs.order_by(PictureCatalog.create_time.desc())
        for picture_catalog in picture_catalogs:
            ...
            data_link_info['creator'] = picture_catalog.creator.name
            result.append(data_link_info)
        database_datas = application.database_data.order_by(DatabaseData.create_time.desc())
        for database_data in database_datas:
            ...
            data_link_info['creator'] = database_data.creator.name
            result.append(data_link_info)
        return result

    @classmethod
    def name_verify(cls, application_id: int, data_name: str) -> bool:
        try:
