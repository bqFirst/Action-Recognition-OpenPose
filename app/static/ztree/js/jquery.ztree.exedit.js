            ...
            onMousedownNode: function(eventMouseDown, _node) {
                        ...
                        if (tmpTarget && tmpTarget.attr("id") != targetSetting.treeId) {
                            ...
                            if (!canPrev && !canNext && !canInner) {
                                ...
                                if ((prevPercent == 1 || dY_percent <= prevPercent && dY_percent >= -.2) && canPrev) {
                                    dX = 1 - tmpArrow.width();
                                    dY = tmpTop - tmpArrow.height() / 2 - 5;
                                    moveType = consts.move.TYPE_PREV;
                                } else if ((nextPercent == 0 || dY_percent >= nextPercent && dY_percent <= 1.2) && canNext) {
                                    dX = 1 - tmpArrow.width();
                                    dY = (tmpNextA == null || (tmpTargetNode.isParent && tmpTargetNode.open)) ? (tmpTop + tmpTargetA.height() - tmpArrow.height() / 2 - 2) : (tmpNextA.offset().top - tmpArrow.height() / 2 - 2);
                ...
                function _docMouseUp(event) {
                    ...
                    if (tmpTarget) {
                        ...
                        $('.rel-tree2-tips').addClass('hide');

                        var first_node = null;
                        if (event.toElement.id === 'rel-tree2') {
                            first_id = $(event.toElement.childNodes[0]).attr('id');
                            first_node = data.getNodeCache(targetSetting, first_id);
                        }

                        //var dragTargetNode = tmpTargetNodeId == null ? null : data.getNodeCache(targetSetting, tmpTargetNodeId);
                        var dragTargetNode = tmpTargetNodeId == null ? first_node : data.getNodeCache(targetSetting, tmpTargetNodeId);
                        ...
                        //$('.rel-tree2-tips').removeClass('hide');
