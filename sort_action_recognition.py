...
# coding=utf-8
# @Time   : 2020/3/26 0026 15:41
# @Author : wangw
# @File   : sort_action_recognition.py
# @Desc   :
...
from app.main.services.core.data.data_file.data_file_operator_container import \
    DataFileContainerOperator as DataFileOperator
from app.main.services.core.template import CaseContainerTemplate
...
def get_command_line_arguments():
    ...
    def parse_args():
        ...
        parser.add_argument("-m", "--model_path", required=False,
                            default='model/trained_classifier_wangw.pickle')
...
def humans_to_skels_list(humans, scale_h = None, img_size=None):
...
multiperson_tracker = None
multiperson_classifier = None
yolo = None


class ActionRecognition(CaseContainerTemplate):

    def init_name(self):
        self.Name = '实时行为识别'

    def init_param(self):
        self.Param = [{'mode': 'video_recognition',
                       'func': 'predict_video',
                       'param': {'input':[{'name': 'video_name',
                                           'type': 'video',
                                           'required': True,
                                           'description': '实时行为识别',
                                           }, ],
                                 'output': [{'name': 'video_name',
                                             'type': 'video',
                                             'description': '实时行为识别结果',
                                             }, ]}},
                      ]

    def init_model(self):
        # -- Detector, tracker, classifier
        # global skeleton_detector
        ...
        global yolo
        yolo = YOLO()
        # global multiperson_tracker
        # multiperson_tracker = Tracker()
        global tracker
        tracker = Sort(20, 3)

        global multiperson_classifier
    ...
    def init_key(self):
        self.Key = 'action_recognition'

    def predict_video(self, video_name: str):
        # cap = cv2.VideoCapture(os.path.join(DataDirectoryPath.get_case_path(), video_name))
        # if not cap.isOpened():
        #     raise Exception("Error opening video stream or file")
        #
        # # -- Image reader and displayer

        images_loader = select_images_loader(SRC_DATA_TYPE, os.path.join(DataDirectoryPath.get_case_path(), video_name))
        ...
        # video writer
        video_name = get_uuid_name(suffix='avi')
        video_writer = lib_images_io.VideoWriter(
            os.path.join(DataDirectoryPath.get_case_path(), video_name), DST_VIDEO_FPS)
        ...
        try:
            ...
            while images_loader.has_image():
                ...
                scale_h = 1.0 * height / width

                # print("joints骨骼点: \n", joints)
                skeletons, scale_h = humans_to_skels_list(joints, scale_h, (width, height))
                ...
                # print("dict_id2skeleton {}".format(dict_id2skeleton))
                ...
                # -- Draw
                img_disp = draw_result_img(img_disp, ith_img, humans, dict_id2skeleton, skeleton_detector,
                                           multiperson_classifier, dict_id2label, scale_h)
